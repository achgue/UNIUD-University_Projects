...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname Es3_conversione_Bx_-_B10) (read-case-sensitive #t) (teachpacks ((lib "drawings.ss" "installed-teachpacks"))) (htdp-settings #(#t constructor repeating-decimal #f #t none #f ((lib "drawings.ss" "installed-teachpacks")) #f)))
; esegue la conversione di un numero in B2 in B10
(define rep->number ; val: intero (questo è una modifica del bin rep)
  (lambda(b num)    ; b:stringa di caratteri della base  num: numero da convertire in B10
    (if(decimal? num)
        (* (sign num) (conv b (no-sign num)))
        (* (sign num) (int b (no-sign num)))
       )
   )) 

;verifico se ho la virgola
(define decimal?   ;vero se ha la virgola
  (lambda (num)
    (let((k (-(string-length num ) 1)))
     (if(string=? (substring num k ) ".")
        ...
        (if(= k 0)
           false
           (decimal? (substring num 0  k )))
           ))
    ))

;a fine operazione mette il segno al risultato
(define sign   ;val: intero 
  (lambda (num) ;stringa
...
(define base-dec
  (lambda (b val)
    (if (string=? val (substring b 0 1))
        0
        (/ (+ (int (substring b 1) val) 1) (string-length b))
        )
    ))
