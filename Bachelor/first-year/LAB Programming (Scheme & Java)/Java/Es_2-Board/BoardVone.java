...
public class BoardVone {
  ...
  private final IntSList codRighe, codColonne, diagDestra, diagSinistra;
  ...
  public Board( int n ) {                             // scacchiera vuota
    ...
    codRighe = new IntSList();
    codColonne = new IntSList();
    diagDestra = new IntSList();
    diagSinistra = new IntSList();
    config = " ";
  }
  ...
  public boolean underAttack( int i, int j ) {        // posizione <i,j> minacciata?
      for(int k = 0; k < queens; k++){
        if(codRighe.listRef(k)==i){
            return true;
        }else if (codColonne.listRef(k)==j){
            return true;
        }else if(diagDestra.listRef(k)==i-j){
            return true;
        }else if(diagSinistra.listRef(k)==i+j){
            return true;
        }   
    }
