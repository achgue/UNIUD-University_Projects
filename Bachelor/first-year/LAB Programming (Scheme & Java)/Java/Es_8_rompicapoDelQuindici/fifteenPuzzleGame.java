...
{
    public fifteenPuzzleGame(int n){
        v1Board b = new v1Board(n);
        ...
        while(!b.isOrdered()){
            ...
            for(int i = 1; i <= b.rowSize; i++){
                for(int j = 1; j <= b.rowSize; j++){
                    if(b.board[k] == t && b.canMove(k) && moves == 0){ //?portare fuori condizione di spostamento e mandenere solo aggionamento parametri interni ai for
                        //if(){
                        moves = 1; //abbiamo fatto una mossa (seve per fare in modo che ci sia una mossa per turno)
                        nToMove[0] = i;
                        nToMove[1] = j;
                        b.move(k);//aggiorno l'array
